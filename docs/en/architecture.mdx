...
sequenceDiagram
    ...
    Browser->>Platform: Click Web CLI in the top-right corner
    ...
    Platform->>Platform: Apply platform audit processing
    Platform->>WebCLI: Forward the WebSocket request
    WebCLI->>WebCLI: Authenticate the request
    ...
    Note over WebCLI,API: Idle sessions are removed after the fixed 15-minute idle timeout.
```
...
1. Click **Web CLI** in the top-right corner of the platform page.
...
4. The platform audit middleware processes the WebSocket request and forwards it to web-cli.
